...
Vector<double> OpenNNClassifier::obtainOutputs (Vector<double> vector_input) {
	...
	try {
		...
		if (debug) { cout << " Input size :" << vector_input.size() << endl; }
		if (debug) { cout << " NN Inputs Number:" << neural_network_Pointer->get_inputs_number() << endl; }
		Vector<double> output_vector = neural_network_Pointer->calculate_outputs(vector_input);

		if (debug) {
			cout << "\n Activations/Outputs : " << output_vector.to_string() << "\n\n" << endl;
		}
